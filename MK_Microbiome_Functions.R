...
run_Maaslin2 <- function(ps_obj, taxa_level, group, analysis_method, normalization, transform, plot_colors, plot_type, qval_threshold, plot_heights){
  ...
  #PLOT EVERYTHING
    ...
    maaslin_res_filt <- maaslin2_results$results %>% as.data.frame() %>% filter(qval <= qval_threshold) %>%
        dplyr::mutate(Enrichment = ifelse(coef > 0, "control", "abnormal"))
...
runANCOM <- function(ps_obj, tax_level, group, Log2FC_cutoff=NULL, name_of_saved_results=NULL, plot_type, plot_heights, plot_colors){
  ...
  df_fig <- ancombc2_results$res %>%
    ...
    dplyr::mutate(Enrichment = ifelse(!!sym(lfc_col_name) > 0, "Control", "Abnormal"),
...
run_aldex2 <- function(ps_obj, group, tax_rank, method, transform, normalization){
  ...
  df_fig <- aldex2_res %>% 
      ...
      dplyr::mutate(Enrichment = ifelse(ef_aldex > 0, "Control", "Abnormal"))
...
combine_DA <- function(maaslin2_results, ancombc2_results, aldex2_results, group, tax_level, plot_heights){
  ...
  #MAASLIN2
    ...
    df_fig_med <- DA_results_df %>%
      ...
      mutate(Enrichment =ifelse(Maaslin2_coef > 0, "Control", "Abnormal")) %>%
...
plot_beta <- function(lda_result, n_top_topics){
    top_terms <- result$b_df %>% 
      ...
      top_n(n_top_topics, beta) %>% #takes the words with the top 10 beta scores
